...
interface FilingListProps {
  ...
  onFilingClick?: (filing: FilingListItem) => void;
  onFilingEdit?: (filing: FilingListItem) => void;
  onFilingDelete?: (filing: FilingListItem) => void;
  onNewFiling?: () => void;
  onExport?: () => void;
  onStatusChange?: (filing: FilingListItem, newStatus: FilingStatus) => void;
  showActions?: boolean;
  className?: string;
}
...
function getPriorityBadge(priority: FilingListItem['priority']) {
  const variants: Record<FilingListItem['priority'], 'danger' | 'warning' | 'primary' | 'secondary'> = {
    CRITICAL: 'danger',
    HIGH: 'warning',
    MEDIUM: 'primary',
    LOW: 'secondary',
...
function getDaysUntilDeadline(dueDate: Date, status: FilingStatus): { days: number; label: string; variant: 'danger' | 'warning' | 'success' | 'secondary' } {
  if (['COMPLETE', 'EFFECTIVE', 'SUBMITTED'].includes(status)) {
    return { days: 0, label: 'Filed', variant: 'success' };
  }
...
function getStatusActions(status: FilingStatus): { label: string; status: FilingStatus; icon: React.ReactNode }[] {
  switch (status) {
    case 'DRAFT':
      return [
        { label: 'Send for Review', status: 'INTERNAL_REVIEW', icon: <Send className="mr-2 h-4 w-4" /> },
      ];
    case 'INTERNAL_REVIEW':
      return [
        { label: 'Send for External Review', status: 'EXTERNAL_REVIEW', icon: <Send className="mr-2 h-4 w-4" /> },
        { label: 'Return to Draft', status: 'DRAFT', icon: <RefreshCw className="mr-2 h-4 w-4" /> },
      ];
    case 'EXTERNAL_REVIEW':
      return [
        { label: 'Mark as Submitted', status: 'SUBMITTED', icon: <CheckCircle2 className="mr-2 h-4 w-4" /> },
        { label: 'Return to Internal Review', status: 'INTERNAL_REVIEW', icon: <RefreshCw className="mr-2 h-4 w-4" /> },
      ];
    case 'SUBMITTED':
      return [
        { label: 'SEC Comment Received', status: 'SEC_COMMENT', icon: <AlertTriangle className="mr-2 h-4 w-4" /> },
        { label: 'Mark as Effective', status: 'EFFECTIVE', icon: <CheckCircle2 className="mr-2 h-4 w-4" /> },
        { label: 'Mark as Complete', status: 'COMPLETE', icon: <CheckCircle2 className="mr-2 h-4 w-4" /> },
      ];
    case 'SEC_COMMENT':
      return [
        { label: 'Response Filed', status: 'RESPONSE_FILED', icon: <Send className="mr-2 h-4 w-4" /> },
      ];
    case 'RESPONSE_FILED':
      return [
        { label: 'Additional Comment Received', status: 'SEC_COMMENT', icon: <AlertTriangle className="mr-2 h-4 w-4" /> },
        { label: 'Mark as Effective', status: 'EFFECTIVE', icon: <CheckCircle2 className="mr-2 h-4 w-4" /> },
        { label: 'Mark as Complete', status: 'COMPLETE', icon: <CheckCircle2 className="mr-2 h-4 w-4" /> },
      ];
    default:
...
export function FilingList({
  ...
  const [filterPriority, setFilterPriority] = useState<FilingListItem['priority'] | 'all'>('all');
      ...
      <CardHeader className="border-b border-slate-200">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <option value="COMPLETE">Filed</option>
          </select>

          <select
            value={filterPriority}
            onChange={(e) => setFilterPriority(e.target.value as FilingListItem['priority'] | 'all')}
            className="rounded-md border border-slate-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          >
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell header>Responsible</TableCell>
          ...
          <TableBody>
            {filteredFilings.length === 0 ? (
              ...
              filteredFilings.map((filing) => {
                const deadline = getDaysUntilDeadline(filing.dueDate, filing.status);
                const statusActions = getStatusActions(filing.status);
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 truncate max-w-[200px]">{filing.title}</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                          ...
                          {statusActions.length > 0 && (
                            <>
                              ...
                              {statusActions.map((action) => (
                                <DropdownItem
                                  key={action.status}
                                  onClick={() => onStatusChange?.(filing, action.status)}
                                >
                                  {action.icon}
                                  {action.label}
                                </DropdownItem>
